...
def test():
    files = [f[:-4] for f in os.listdir('../../CHI18/OUTPUT') if f.endswith('.png')]
    files = [f.replace("/", ":") for f in files]
    conn = sqlite3.connect(DB_FILE_NAME)
    if conn is not None:
        c = conn.cursor()
        for f in files:
            c.execute("SELECT COUNT(*) FROM variant_output WHERE variant = ?", [f])
            count = c.fetchone()[0]

            if count == 0:
                print "Unnecessary variant: ", f

        c.execute("SELECT variant FROM variant_output")
        variants = c.fetchall()
        missing = [v[0] for v in variants if v[0] not in files]
