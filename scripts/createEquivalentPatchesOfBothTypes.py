...
CREATE_PATCHES_TABLE_QUERY = 'CREATE TABLE VARIANTS_TO_FUNCTIONS(FQN VARCHAR(30), START INTEGER, END INTEGER, BODY TEXT, UUID VARCHAR)'
FUNCTION_INSERT_QUERY = 'INSERT INTO VARIANTS_TO_FUNCTIONS VALUES (?,?,?,?,?)'
UPDATE_PATCHES_QUERY = 'UPDATE VARIANTS_TO_FUNCTIONS SET END = ? WHERE END = ? AND FQN = ? AND BODY = ?'

CREATE_FILE_TABLE_QUERY = 'CREATE TABLE VARIANTS_TO_FILES(FQN VARCHAR(30), START INTEGER, END INTEGER, UUID VARCHAR)'
	...
	for i in range(0, len(variantNames)):
		...
		for file in files:
			...
			if isEquivalentToPrevVariant:
				insertFileToDb(conn, fileFqn, variantIndex, variantIndex-1)
			else:
				insertFileToDb(conn, fileFqn, variantIndex)
